...
const ProductList = () => {
    ...
    const TableName = 'Maestro Constantes de calculo';
    ...
    const fetchData = async (accessToken) => {
        try {
            const jsonData = await ConstHelper.fetchData();

            setRows(jsonData);
        } catch (error) {
            console.log('ConstCalculo.fetchData::erro: '.error);
    ...
    const handleDelete = async (id) => {
        ...
        await ConstHelper.deleteDataById(id);
        // para actualizar el componente
        SetActualizacion(true);
        console.log(`Constante con ID ${id} ha sido eliminada`);
    };

    const handleCreateAPI = async (newData) => {
        await ConstHelper.createData(newData);
    };
    // Función para actualizar la API utilizando
    const handleUpdateAPI = async (id, data) => {
        await ConstHelper.updateDataById(id, data);
    };
