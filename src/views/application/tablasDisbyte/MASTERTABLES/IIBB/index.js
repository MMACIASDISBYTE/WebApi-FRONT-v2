...
import { IIBBHelper } from '../../../../../helpers/IIBBHelper';
...
const ProductList = () => {
    ...
    const fetchData = async (accessToken) => {
        try {
            const jsonData = await IIBBHelper.fetchData();
    ...
    const handleDelete = async (code) => {
        ...
        await IIBBHelper.deleteDataByCode(code);
        // para actualizar el componente
        SetActualizacion(true);
        console.log(`IIB con codigo ${code} ha sido eliminado`);
    };

    const handleCreateAPI = async (newData) => {
        await IIBBHelper.createData(newData);
    };
    // Función para actualizar la API utilizando
    const handleUpdateAPI = async (id, data) => {
        await IIBBHelper.updateDataByCode(id, data);
    };
