...
const ProductList = () => {
    ...
    const fetchData = async (accessToken) => {
        try {
            const jsonData = await CustodiaHelper.fetchData();
    ...
    const handleDelete = async (id, description) => {
        ...
        await CustodiaHelper.deleteDataById(id);
        // para actualizar el componente
        SetActualizacion(true);
        console.log(`El Custodia ${description} con ${id}, ha sido eliminado`);
    };

    const handleCreateAPI = async (newData) => {
        await CustodiaHelper.createData(newData);
    };
    // Función para actualizar la API utilizando
    const handleUpdateAPI = async (id, data) => {
        await CustodiaHelper.updateDataById(id, data);
    };
