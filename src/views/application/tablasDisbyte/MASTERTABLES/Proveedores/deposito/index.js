...
const ProductList = () => {
    ...
    const fetchData = async (accessToken) => {
        try {
            const jsonData = await DepositoHelper.fetchData();
    ...
    const handleDelete = async (id, description) => {
        ...
        await DepositoHelper.deleteDataById(id);
        // para actualizar el componente
        SetActualizacion(true);
        console.log(`El Deposito ${description} con ${id}, ha sido eliminado`);
    };

    const handleCreateAPI = async (newData) => {
        await DepositoHelper.createData(newData);
    };
    // Función para actualizar la API utilizando
    const handleUpdateAPI = async (id, data) => {
        await DepositoHelper.updateDataById(id, data);
    };
