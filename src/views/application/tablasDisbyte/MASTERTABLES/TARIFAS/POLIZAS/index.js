...
const ProductList = () => {
    ...
    const fetchData = async (accessToken) => {
        try {
            const jsonData = await TarifasPolizaHelper.fetchData();
    ...
    const handleDelete = async (id) => {
        ...
        await TarifasPolizaHelper.deleteDataById(id);
        // para actualizar el componente
        SetActualizacion(true);
        console.log(`Tarifa POLIZA con ID ${id} se ha sido eliminada`);
    };

    const handleCreateAPI = async (newData) => {
        await TarifasPolizaHelper.createData(newData);
    };
    // Función para actualizar la API utilizando
    const handleUpdateAPI = async (id, data) => {
        await TarifasPolizaHelper.updateDataById(id, data);
    };
