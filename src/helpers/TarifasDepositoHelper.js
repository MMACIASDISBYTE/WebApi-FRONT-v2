...
export const TarifasDepositoHelper = {
  baseUrl: process.env.REACT_APP_BASE_URL,
  rutaTabla: "TarifasDeposito",
  constructor(baseUrl) {
  ...
  fetchDataPais: async function () {
    try {
      ...
      // console.log("TarifasDepositosHelper/vista.fetchData::response", response);
  ...
  readDataByDepByCont: async function (dep, cont) {
    try {
      const response = await fetch(
        `${this.baseUrl}/${this.rutaTabla}/${dep}/${cont}`
      );
      const jsonData = await response.json();
      return jsonData;
    } catch (error) {
