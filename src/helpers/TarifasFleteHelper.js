...
export const TarifasFleteHelper = {
    baseUrl: process.env.REACT_APP_BASE_URL,
    rutaTabla: 'TarifasFlete',
    constructor(baseUrl){
 ...
 //Consulta a la API VIEJA
    ...
    fetchData: async function () {
        try {
            ...
            console.log('TarifasFleteHelper.fetchData::response', response);
    ...
    readDataByDepByCont: async function(dep,cont) {
        try {
            const response = await fetch(`${this.baseUrl}/${this.rutaTabla}/${dep}/${cont}`);
            const jsonData = await response.json();
            return jsonData;
        } catch (error) {
            console.error('Error', error);
